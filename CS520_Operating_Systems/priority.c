...
// Insert a descending order based on priority
void insertNodeBasedOnPrioritySchedulingAlgorithm(LinkedList* list, ProcessNode* newNode) {
	...
	// If the new node should be inserted after nodes with the same priority or as the new head
	else {
		ProcessNode* current = list->head;
		ProcessNode* prev = NULL;
		// Find the appropriate position to insert the new node
		while (current != NULL && current->dataVal.priority >= newNode->dataVal.priority) 
		{
			prev = current;
		...
		// Insert the new node after the nodes with the same priority or in between other nodes
		else {
...
// Function to print the processes of the linked list and remove them
void printAndRemoveProcesses(LinkedList* list) {
	...
	// After removing all processes, update the tail to NULL
	list->tail = NULL;
}
...
int main(int argc, char* argv[]) {
	...
	while (fgets(line, sizeof(line), file)) {
		...
		if (result < 3) {
			printf("Error. The cpu burst or priority is not a number or an entry for either of those is missing.");
			fclose(file);
			exit(1);
		} else if (result > 3) {
		...
		// Create a new process and insert it into the linked list
		ProcessNode* newNode = createNode(process);
		insertNodeBasedOnPrioritySchedulingAlgorithm(list, newNode);	
	}

	// Close the file
	fclose(file);

	// Remove the processes from the list and print them
