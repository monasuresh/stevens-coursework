...
async function checkIfEventIdExists(eventId) {
    ...
    const foundEvent = await eventCollection.findOne({ _id: new ObjectId(eventId) });

    if (!foundEvent) throw 'Event not found';
}

async function validateMaxCapacityAndDuplicateEmails(eventId, attendeeParams) {
    const eventCollection = await events();

    const eventToModify = await eventCollection.findOne({ _id: new ObjectId(eventId) });
    ...
    const existingAttendee = await eventToModify.attendees.find(attendee => attendee.emailAddress === attendeeParams.emailAddress);
    if (existingAttendee) {
        throw 'An attendee with the same email address already exists in the event.';
    }
}
...
export default {
    ...
    validateMaxCapacityAndDuplicateEmails
}