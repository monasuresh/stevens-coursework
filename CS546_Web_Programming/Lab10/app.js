...
import session from 'express-session';  // Assuming you are using express-session for sessions
...
app.use('/', async (req, res, next) => {
    ...
    console.log(`${timestamp}: ${req.method} ${req.originalUrl} (${req.session.user ? 'Authenticated User' : 'Non-Authenticated User'})`);
    if (req.originalUrl === '/') {
        if (req.session.user) {
            if (req.session.user.role === 'admin') {
                // Redirect admin to /admin route if not already there
                ...
                // Redirect user to /protected route if not already there
